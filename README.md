...
The repository's coding standards are documented in
[`.github/copilot-instructions.md`](.github/copilot-instructions.md). In particular,
comments should explain intent rather than restate code, exported functions in
`db/` and `src/lib/` require TSDoc/JSDoc for their purpose, parameters, and
return values, and reusable Astro components must document their `Props`
interfaces. Selected TypeScript formatting conventions and exported API type
rules are enforced by ESLint; see the
[data-layer](.github/instructions/drizzle.instructions.md) and
[Astro](.github/instructions/astro.instructions.md) instruction files for
